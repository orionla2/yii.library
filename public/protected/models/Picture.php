...
class Picture extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'productPictures' => array(self::HAS_MANY, 'ProductPicture', 'picture_id'),
		);
	}
    ...
    public function insertAllPictures ($pictArr) {
        $retArr = array();
        foreach ($pictArr as $path) {
            $model = new Picture();
            $model->path = $path;
            $model->save();
            $retArr[] = Yii::app()->db->getLastInsertID();
        }
        return $retArr;
    }
}
